...
The active Codex task calls `session-cli.js wait` as a bounded long-poll and heartbeat. Learner chat, activity attempts, help, and stop events arrive as ordered structured events. Codex sends tutor messages and validated `activity.inline` events through the same CLI. The browser renders them chronologically and keeps prior sandboxed activities in the in-memory session history. The harness makes no direct OpenAI API call.
